...
main(int argc, char *argv[]) {
  ...
  printf("insert 194M, time each 5M\n");
  struct data rep;
  int cmd;
  ...
  if (argc != 2) {
    ...
    fpnEdgesInsert=fopen("./results/nEdgesInsert.txt", "w");
    fpinsert=fopen("./results/fpinsert.txt", "w");

    fpnEdgesDelete=fopen("./results/nEdgesDelete.txt", "w");
    fpdelete=fopen("./results/fpdelete.txt", "w");

    fptotalSizeCount=fopen("./results/totalSizeCount.txt", "w");

    if (fpnEdgesInsert == NULL ||fpinsert==NULL || fpnEdgesDelete == NULL ||  fpdelete==NULL || fptotalSizeCount==NULL ){
  ...
  clock_t beginning = cpuTime(),start,start2,diff=0,diff2=0;
  long edgeCount=0;
  long deleteEdgeCount=0;
  long nedgesstartdelete=0;
  bool deleting=false;
  ...
  while (1) {
	...
	if (edgeCount%5000000 == 0 && edgeCount!=0) {

	    fprintf(fpnEdgesInsert,"%lu,\n",edgeCount);
	    fprintf(fpinsert,"%f,\n",(float) ((diff*1000)/CLOCKS_PER_SEC)/edgeCount);
		...
		char intbuf[100];
		sprintf(intbuf, "%i", totalSizeCount);
		strcat(auxpath,intbuf);
	 	strcat(auxpath,"/");
		struct stat st = {0};

		if (stat(auxpath, &st) == -1) {
		    mkdir(auxpath, 0700);
		}

		strcat(auxpath,"hashsizes");

		FILE* fout = fopen(auxpath, "w");
		...
		for (i = 0; i <= rep.maxr; i++) {
			if (rep.k2t[i] != NULL) {
				...
				sprintf(intbuf, "t.%d", i+1);
				strcat(auxpath,intbuf);

				saveRepresentation(rep.k2t[i], auxpath);
			}
		}
        }
	...
	if (deleting==false){
		diff=0;
		diff2=0;
		nedgesstartdelete=edgeCount;		
		edgeCount=edgeCount-10000000;
		deleteEdgeCount=0;
	...
	if (deleteEdgeCount%5000000 == 0 && deleteEdgeCount!=0) {

	    fprintf(fpnEdgesDelete,"%lu,\n",nedgesstartdelete);
	    fprintf(fpdelete,"%f,\n",(float) ((diff2*1000)/CLOCKS_PER_SEC)/deleteEdgeCount);
