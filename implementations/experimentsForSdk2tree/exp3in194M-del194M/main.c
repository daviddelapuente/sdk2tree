...
main(int argc, char *argv[]) {
  ...
  printf("delete 194M, time each 5M\n");
  struct data rep;
  int cmd;
  ...
  if (argc != 2) {
    ...
    fpdeletesize=fopen("./results/deletesize.txt", "w");

    if (fpnEdges == NULL ||fpdelete==NULL || fpdeletesize==NULL){
  ...
  clock_t beginning = cpuTime(),start,diff=0;
  long edgeCount=0;
  while (1) {
    /* printf("[%f] > ", cpuTime()); */
    cmd = getchar();
	...
	if(edgeCount%5000000==0 && edgeCount!=0){
	    ...
	    float totalSize=0;
            for (i = 0; i <= rep.maxr+1; i++){
                totalSize+=MAXSZ(max(rep.nv,rep.ne), i);
	    }
	    totalSize+=rep.htsz;
	    totalSize+=rep.elsz;
            totalSize+=rep.adjsz;

	    fprintf(fpdeletesize,"%f,\n",totalSize);
	...
	fprintf(fpdeletesize,"%f,\n",totalSize);
		...
		FILE* fout2 = fopen(auxpath, "w");
		fwrite(&(rep.nv), sizeof(uint32_t), 1, fout2);
		fwrite(&(rep.ne), sizeof(uint32_t), 1, fout2);
		fwrite(&(rep.eln), sizeof(uint32_t), 1, fout2);

		for (i = 0; i < rep.eln; ++i)
			fwrite(&(rep.elst[rep.efree[i]]), sizeof(uint32_t), 2, fout2);
		fclose(fout2);
